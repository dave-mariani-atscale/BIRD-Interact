...
RESULT_SHAPE_TIP = (
    ...
    "- Row ORDER is part of that comparison, and NOT only when the question sounds like a ranking. Whenever a ranking IS implied — 'top', 'best/worst', 'highest/lowest', 'most/least' — always add an explicit ORDER BY on the measure being ranked, even if the question does not say 'sorted by'. But a plain multi-row listing is order-compared too, and assuming otherwise is a measured way to lose a task whose rows were already correct: a listing of per-center events returned exactly the right rows and still failed, because it sorted by the timestamp alone where the reference sorted by the center and then the timestamp. So whenever the answer has more than one row, sort COARSE TO FINE — first the entity the report is organized around (the id, center, region or category the question groups by), then the key that orders rows within it (time, rank, or the measure) — rather than by a time column on its own. A sensible ORDER BY never costs you anything and its absence can fail an otherwise correct answer."
)
