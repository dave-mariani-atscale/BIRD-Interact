...
ASK_USER_TIP = (
    ...
    "- The same applies when the rows themselves are named categories rather than a single classification column — one row per HLA locus, per score band, per status. The reference prints label text you cannot derive ('A-Locus Mismatch', not 'A'), so ask for the exact wording of every row label. And when the question names the categories in a particular sequence, that sequence is usually the column or row order the reference uses: do not silently re-sort them alphabetically, and ask which order is wanted if the question does not make it plain. That holds for labels the reference PRINTS and you must spell; when the categories are simply the values of a stored column and the question asks ONE FIGURE per category ('break down sales by simple / medium / complex routes and show me their counts'), it is the one-figure-per-group listing below - sort by the figure, largest first, not in the order the words came (measured: the same task passed sorted by the count and failed sorted in the question's sequence).\n"
    ...
    "- When part of the question asks for something that is NOT a column - 'and what's their status', 'add info about the delivery routes', 'include their purchase rates and how often they hide their identity' - the CONTENT of that part is an open slot exactly like an unstated cutoff: it may be one column or a whole breakdown (several counts, several percentages), and the reference decides. Ask once, BEFORE the first submit, for the COMPLETE list of columns each result row should contain ('what exactly should each row of the result show, from left to right?') - not 'which column is their status, this one or something else?': a question that offers one column gets that column confirmed and nothing more, while a question about the whole row gets the whole row (measured on one task over three runs: the narrow ask was answered with the one field it named every time and the phase failed; the one run whose ask was about the whole output was answered with the field plus five counts and passed - the ask does not always open the slot, but nothing else ever has). Then build exactly that. Do not spend the ask when the words name a column ('their operational status', 'the count'). Measured: a listing of platforms 'and their status' was two columns in the agent's reading and seven in the reference (the status plus five alert counts); the run that asked got the column list in one turn and passed phase 1, the run that did not ask submitted three shapes and failed all three.\n"
    "- When a listing needs an identifier column and the model publishes both a unique code/key and a display name for the entity, use the code and do not spend an ask on the choice: a name shared by several entities cannot identify a row, and the descriptions say which is unique. Show the name only when the question says 'name'. Measured: two listings on one database each spent 2 coins asking 'name or code?' and were told the code both times.\n"
    ...
    "- A PROJECTED, potential, expected, estimated or lifetime figure ('the potential financial hit', 'the projected revenue loss', 'the expected loss over its life') is a FORMULA, not a recorded column: it is a rate times an assumed volume, price and horizon, and the constants live with the user. A recorded historical amount with a similar name (a logged revenue loss, a booked cost) is a different quantity and never the answer to a projection. When no published calculation and no glossary entry defines the projection, ask for its formula and constants outright ('what formula and constants define the projected loss?') before the first submit - do not substitute the recorded amount (measured: a task summed the logged revenue loss where the answer was a degradation rate times three constants, twice).\n"
    "- When a PER-RECORD rate or score (one value per snapshot, reading, visit, event) is summed or averaged 'across the plants' / 'across the assets' / 'for these customers', the GRAIN is an open slot: every record of every qualifying entity counts, or each entity is first reduced to one row (its latest record, or its own average) and those are combined. The two figures differ whenever entities have unequal record counts, and only the user knows which was meant. Ask once, as a closed question ('every snapshot of each plant, or one value per plant - and if one, the latest?'), before choosing a reduction (measured: a task reduced to the latest snapshot per plant where every snapshot counted, and the sum was 8 percent off with every other part right).\n"
)
