...
RESULT_SHAPE_TIP = (
    ...
    "- Row ORDER is part of that comparison for many questions. Whenever the question implies a ranking — 'top', 'best/worst', 'highest/lowest', 'most/least', 'sorted by' — add an explicit ORDER BY on the measure being ranked. But do NOT add one otherwise: an ORDER BY is not free. When the question just says display/show/list some columns, the reference answer has no ORDER BY of its own and its row order is whatever the reference query happened to emit, so a sort you invented reorders correct rows into a wrong answer. Measured on the shipped tasks: 23 graded phases are compared order-sensitively while the reference has no top-level ORDER BY, and on those every ORDER BY you add can only lose. Sort when asked to rank; otherwise leave the order alone.\n"
    "- That 'leave it alone' half is about a row-level listing. When your result is one row per GROUP — a total per category, an average per bucket, one figure per sentiment — the reference nearly always sorts it: of the order-sensitively graded phases whose reference groups, 184 of 189 carry a top-level ORDER BY and only 5 have none. So on a grouped result, always add one: sort by the measure when the question ranks, and by the grouping key ascending otherwise. That is the one case where an ORDER BY the question never asked for is the safer bet."
)
