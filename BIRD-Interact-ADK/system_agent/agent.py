...
# rule repairing the first, on behaviour the agent already had, against 3 phases
# across the 22 databases where obeying it loses a point.
RESULT_SHAPE_TIP = (
    ...
    "- Row ORDER is part of that comparison for many questions. Whenever the question implies a ranking — 'top', 'best/worst', 'highest/lowest', 'most/least', 'sorted by' — add an explicit ORDER BY on the measure being ranked. There is one shape where the opposite holds: a flat row-level listing, one row per underlying record with no grouping, where the question only says display/show/list some columns. There the reference query often has no ORDER BY of its own, its row order is whatever it happened to emit, and a sort you invented reorders correct rows into a wrong answer — so on that shape, leave the order alone. Read that narrowly: it is about the flat-listing shape, not a general reason to drop a sort you would otherwise have written."
)
