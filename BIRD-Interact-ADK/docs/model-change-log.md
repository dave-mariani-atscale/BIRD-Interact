...
| Q-17b | **Republishing over an already-published catalog appends duplicate model relations instead of replacing them.** Once duplicated, the metadata path dies with `ERROR: relation "<model>" already exists` while the QUERY path keeps working - `run_query` returns correct numbers, but `list_models`, `explore_columns` and `focus_columns` all fail. An agent is then blind but not obviously broken, which reads as a model regression rather than an infrastructure fault. Redeploying again does NOT clear it: the error simply names a different duplicate (observed moving from `Exchange Traded Funds` to `Cybermarket Pattern` after a second publish). | Delete/unpublish the catalog in the engine, then publish once cleanly - never deploy on top. Before any run, gate on `list_models` succeeding AND returning the expected model count; a successful `run_query` is NOT sufficient evidence that the catalog is healthy. | Yes - hit twice on 2026-08-10/11. First occurrence voided three atscale runs (every task scored 0.000, 81-84% of tool calls erroring). `scripts/summarize_runs.py` now flags such runs as SUSPECT and excludes them rather than reporting a lift from them. |
